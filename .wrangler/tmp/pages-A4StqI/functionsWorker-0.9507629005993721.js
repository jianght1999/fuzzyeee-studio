...
var onRequestGet = /* @__PURE__ */ __name(async (ctx) => {
  ...
  const kvContent = await ctx.env.NOTES_CONTENT.get(`content/${category}/${slug}`);
  return json({ ...row, parentSlug: row.parent_slug, updatedAt: row.updated_at, content: kvContent || row.content });
}, "onRequestGet");

// api/pages/[category].ts
var onRequestGet2 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  const pages = results.map((r) => ({
    ...
    hasChildren: hasKids.has(r.slug)
  }));
  return json({ pages });
}, "onRequestGet");
...
var onRequestGet3 = /* @__PURE__ */ __name(async (ctx) => {
  const key = "img/" + (ctx.params.key || []).join("/");
...
var onRequestPost = /* @__PURE__ */ __name(async (ctx) => {
  ...
  const { category, slug } = parse(body.path);
...
var onRequestPost2 = /* @__PURE__ */ __name(async (ctx) => {
  const body = await getBody(ctx.request);
  if (!checkToken(body.token || "")) return json({ error: "not authenticated" }, 403);
  const { category, slug } = parse2(body.path);
...
var onRequestPost3 = /* @__PURE__ */ __name(async (ctx) => {
  const { password } = await getBody(ctx.request);
  ...
  if (!row) {
    ...
    await ctx.env.DB.prepare("INSERT INTO settings (key,value) VALUES ('password_hash',?)").bind(hash).run();
    return json({ success: true, token: genToken() });
  }
  if (await verifyPw(password, row.value)) {
    return json({ success: true, token: genToken() });
  }
...
var onRequestPost4 = /* @__PURE__ */ __name(async (ctx) => {
  const { token } = await getBody(ctx.request);
  delToken(token);
  return json({ success: true });
}, "onRequestPost");
...
var onRequestPost5 = /* @__PURE__ */ __name(async (ctx) => {
  const body = await getBody(ctx.request);
  if (!checkToken(body.token || "")) return json({ error: "not authenticated" }, 403);
  const o = parse3(body.oldPath), n = parse3(body.newPath);
...
// api/music-list.ts
var onRequestGet4 = /* @__PURE__ */ __name(() => json(["/music/waltz-for-debby.mp3"]), "onRequestGet");
...
var onRequestPost7 = /* @__PURE__ */ __name(async (ctx) => {
  const body = await getBody(ctx.request);
  if (!checkToken(body.token || "")) return json({ error: "not authenticated" }, 403);
  const o = parse4(body.oldPath), n = parse4(body.newPath);
  const kids = await ctx.env.DB.prepare("SELECT slug FROM notes WHERE category=? AND parent_slug=?").bind(o.category, o.slug).all();
  for (const k of kids.results) {
    const ns = n.slug + "/" + k.slug.split("/").pop();
    await ctx.env.DB.prepare("UPDATE notes SET slug=?,category=?,parent_slug=? WHERE category=? AND slug=?").bind(ns, n.category, n.slug, o.category, k.slug).run();
...
var onRequestPost8 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  if (body.content.length > KV_THRESHOLD)
    await ctx.env.NOTES_CONTENT.put(`content/${category}/${slug}`, body.content);
  else
    await ctx.env.NOTES_CONTENT.delete(`content/${category}/${slug}`).catch(() => {
    });
  await ctx.env.DB.prepare(
...
var onRequestPost9 = /* @__PURE__ */ __name(async (ctx) => {
  ...
  if (!checkToken(token || "")) return json({ error: "not authenticated" }, 403);
...
// ../.wrangler/tmp/pages-A4StqI/functionsRoutes-0.8041324928887796.mjs
var routes = [
  {
...
function stringToRegexp(path, keys, options) {
  return tokensToRegexp(parse5(path, options), keys, options);
}
